...
namespace RRTStar
{
    ...
    {
    public:
        rrtStarTree(std::shared_ptr<StateSpace<T>> stateSpace)
        {
        ...
        void initVector(int num, int initW, int initH) {
            ...
            _unitWidth = initW;
            _unitHeight = initH;
            _classifyNodes = new std::vector<Node<T> *>[_numVector];
        }
        
        void addToClassifyNodes(Node<T> *&node){
            int dst = _stateSpace->getLocationDest(node->state(), _unitWidth, _unitHeight);
            (*(_classifyNodes + dst)).push_back(node);
        }
        ...
        void reset(bool eraseRoot = false)
        {
            ...
            for(int i = 0; i < _numVector; i++) {
                if(!(*(_classifyNodes + i)).empty())
                    (*(_classifyNodes + i)).clear();
            }
 ...
 //           qDebug()<<"range="<<range;
            ...
            int dst = _stateSpace->getLocationDest(newnode, _unitWidth, _unitHeight);
            //for(Node<T> *other : _nodes)
            for(Node<T> *other : *(_classifyNodes + dst))
            {
        ...
        void OptimizeNearNodes(Node<T> *newnode , Node<T> *parent)
        {
            for(Node<T> *nearnode : _nearnodes)
            {
                if(nearnode!=parent)
                {
                    ...
                    {
                        //addInform(nearnode,nearnode->parent()->state());
        ...
        virtual Node<T> *extend(double limitdistance,const T &target, Node<T> *source = nullptr) {
            ...
            if (!_stateSpace->transitionValid(source->state(), intermediateState)) {
                ...
                if (source->inform() == nullptr) {
                    (source->inform()) = _stateSpace->nearObstacle(source->state(),maxStepSize());
                }
                std::vector<T> intermediateStates = _stateSpace->intermediateStates(source->state(), source->inform());
                for(T aState : intermediateStates) {
                    if (!_stateSpace->transitionValid(source->state(), aState))
                        continue;
                    double a_distance = _stateSpace->distance (source->state (),aState) + source->getDistance ();
                    if( a_distance > limitdistance)
                        continue;
                    nearnodes(aState);
                    //qDebug()<<_nearnodes.size();
                    Node<T> *oldsource = source;
                    for(Node<T> *nearnode : _nearnodes)
                    {
                        double aa_distance = nearnode->getDistance ()+_stateSpace->distance (nearnode->state (),aState);
                        if(aa_distance<a_distance)
                        {
                            a_distance=aa_distance;
                            source=nearnode;
                        }
                    }
                    Node<T> *a = new Node<T>(aState, source);
                    a->setDistance (a_distance);
                    _nodes.push_back(a);
                    //OptimizeNearNodes(a,source);
                    int relativePositon = _stateSpace->getRelativePosition(oldsource->state(), aState);
                    //qDebug()<<relativePositon;
                    *(oldsource->inform() + relativePositon) = 1;
                ...
                return nullptr;
                //qDebug()<<minStep;
                //intermediateState = _stateSpace->intermediateState(source->state(), d, minStep);
            }
            else {
   ...
   //         qDebug()<<"nearnodes:"<<_nearnodes.size ();
        ...
        /*void calInform(Node<T> *source) {
            std::vector<T> _nearObstacle = _stateSpace->nearObstacle(source->state(),maxStepSize());
            //qDebug()<<"1:"<<_nearObstacle.size();
            for(T obs : _nearObstacle)
            {
                addInform(source,obs);
            }
            
            //for(T child : _nearObstacle)
            //{
            //    addInform(source,obs);
            //}
            //qDebug()<<(source->inform()).size();
        }
        
        void addInform(Node<T> *source) {
            source->inform() = _stateSpace->nearestObstacle(const Vector2f &state);
            //qDebug()<<dis;
            if(dis <= maxStepSize()) {
                
                Infor<T> *newInform = new Infor<T>();
                newInform->minStep = dis;
                newInform->wobs = -dis/maxStepSize();
                T dir = obs - source->state();
                T weightDir = _stateSpace->calWeightDir(dir, newInform->wobs);
                newInform->dobs = weightDir;
                source->addInfor(newInform);
                //qDebug()<<(source->inform()).size();
            }
        ...
        std::vector<Node<T> *>* &classifyNodes() {
            return _classifyNodes;
        }
    ...
    protected:
        ...
        std::vector<Node<T> *> *_classifyNodes;
