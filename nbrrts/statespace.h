...
public:
    ...
    //virtual std::vector<T> nearObstacle(T source, float maxStepSize) = 0;
    virtual int *nearObstacle(const T &state, float maxDist) = 0;
    virtual std::vector<T> intermediateStates(const T &state, const int *inform) = 0;
    virtual int getRelativePosition(const T &state, const T &to) = 0;
    virtual int getLocationDest(const T &state, int initW, int initH) = 0;
